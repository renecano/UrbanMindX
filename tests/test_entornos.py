# ============================================================
# test_entornos.py — VERSION 3
# Actualizado para observacion del semaforo de 9 valores.
# El entorno vehiculo sigue con 6 valores.
# ============================================================
...
# ============================================================
# Tests del semaforo (ahora con 9 valores de observacion)
# ============================================================
...
class TestEntornoSemaforo:
    ...
    def test_reset_regresa_forma_correcta(self, env_semaforo):
        ...
        assert obs.shape == (9,), f"Esperaba (9,), got {obs.shape}"
    ...
    def test_obs_dentro_de_limites(self, env_semaforo):
        ...
        assert np.all(obs >= low),  f"Obs por debajo del minimo: {obs}"
        assert np.all(obs <= high), f"Obs por encima del maximo: {obs}"

    def test_presion_se_calcula_bien(self, env_semaforo):
        """Verifica que presion_NS = norte+sur y presion_EO = este+oeste."""
        obs, _ = env_semaforo.reset()
        autos_n, autos_s, autos_e, autos_o = obs[0], obs[1], obs[2], obs[3]
        presion_ns, presion_eo, diferencia = obs[6], obs[7], obs[8]
        assert presion_ns == autos_n + autos_s
        assert presion_eo == autos_e + autos_o
        assert diferencia == presion_ns - presion_eo
    ...
    def test_cambio_prematuro_no_cambia_fase(self, env_semaforo):
        env_semaforo.reset()
        env_semaforo._tiempo_en_fase = 0
        fase_antes = env_semaforo._fase_verde
...
# ============================================================
# Tests del vehiculo (sin cambios, sigue con 6 valores)
# ============================================================
...
class TestEntornoVehiculo:
    ...
    def test_obs_dentro_de_limites(self, env_vehiculo):
        ...
        assert np.all(obs >= low)
        assert np.all(obs <= high)
    ...
    def test_frenar_no_baja_de_cero(self, env_vehiculo):
        env_vehiculo.reset()
        for _ in range(20):
            obs, _, done, _, _ = env_vehiculo.step(0)
            assert obs[0] >= 0.0
            if done:
                break
